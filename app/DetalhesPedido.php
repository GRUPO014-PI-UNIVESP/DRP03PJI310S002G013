<?php
  ...
  if(!empty($_GET['id'])){ $id = $_GET['id'];
    $pedi = array(); $comp = array(); $rece = array(); $anaM = array(); $fabr = array(); $anaP = array(); $entr = array(); $tota = array(); $mensagem = '';

    $sql0 = $connDB->prepare("SELECT * FROM pedidos WHERE NUMERO_PEDIDO = :numPedido");
    $sql0->bindParam(':numPedido', $id, PDO::PARAM_INT);
    $sql0->execute(); 
    $rowPedido = $sql0->fetch(PDO::FETCH_ASSOC);
    if($rowPedido['ETAPA_PROCESS'] >= 6){
      $baseFabr = (($rowPedido['QTDE_PEDIDO'] / $rowPedido['CAPAC_PROCESS']) * 60);
    ...
    <div class="main"><br>
      <div class="row g-2">
        ...
        </div><?php if($mensagem != ''){ echo $mensagem; } ?>
        <div class="col-md-6">
